...
>轻量级锁：快速响应，当并发量比较高时会升级成重量级锁，会采用自旋锁方式占用cpu，如果长时间获取不了锁会耗费cpu资源，后面的线程进来时会升级成重量级锁。
如果线程A使用对象1处于无锁状态，则在栈针里面copy对象1头部的Mark Word信息到栈针，将指向对象指向头部Mark Word的指针指向栈针的Replace Mark Word，将对象1头部的锁标识置为偏向锁。
...
2.volatile的实现原理
>保证变量的多线程可见性，不能保证原子性。涉及到CPU的多级缓存机制，CPU不会直接操作内存而是先读写最近的缓存，如果多级缓存都没有就会从内存中读取到缓存中。这就是volatile设计的出发点，被标记为volatile的变量被修改时会锁住所在缓存，直接写到内存中去，同时通过嗅探的方式让其他的CPU的多级缓存失效，其他CPU上的线程再读取该变量时会直接从内存中读取。
